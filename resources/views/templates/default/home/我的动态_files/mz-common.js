$(function() {
    ...
    $.get(_lps_site_url+"common/recommend_keyword/")
        .done(function(res) {
            var html = "";
            try {
...
//            console.dir(xhr);
	...
	$(".sendE a").click(function(){
        ...
        $.ajax({
             type: "GET",
             url: _lps_site_url+"user/send_again_email",
...
function refresh_captcha(event){
    $.ajax({
        url: _lps_site_url+"captcha/refresh/?"+Math.random(),
    ...
    return false;
    //$.get(_lps_site_url+"captcha/refresh/?"+Math.random(), function(result){
        //headers: {'X-Requested-With': 'XMLHttpRequest'};
        //$('#'+event.data.form_id+' .captcha').attr("src",result.image_url);
        //$('#'+event.data.form_id+' .form-control-captcha[type="hidden"]').attr("value",result.key);
    //});
        ...
        success: function(data) {
            if(data.account_l){
                ...
                if(data.status == "success"){
                    ...
                    window.location.href = "/common/course";
                    return;
...
function register_form_submit(){
    if(current_register_form == "email_register_form"){
        $.ajax({
            cache: false,
            type: "POST",
            url:_lps_site_url+"user/register/email/",
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url:_lps_site_url+"user/register/mobile/",
...
function find_password_form_submit(){
    $.ajax({
        cache: false,
        type: "POST",
        url:_lps_site_url+"user/password/find/",
...
function mobile_code_password_form_submit(){
    $.ajax({
        cache: false,
        type: "POST",
        url:_lps_site_url+"user/password/find/mobile/",
        ...
        success: function(data) {
            if(data.mobile_code_f){
                ...
                location.href=_lps_site_url+"user/password/reset/"+account+"/"+code;
            }
        },
        complete: function(XMLHttpRequest){
...
function send_sms_code(form_id,tips_id){
    ...
    $.ajax({
        cache: false,
        type: "POST",
        url:_lps_site_url+"user/register/mobile/sendsms_signup/",
...
function get_careercourse(){
    $.ajax({
        type: "GET",
        async: false,
        url: _lps_site_url+"user/get/careercourse/",
        dataType: "json",
        success: function(data){
            var data = eval(data);
...
function create_class_save(){
    ...
    $("#create-class").live("click",function(){
        if($('#select_course').val()==0){
            ...
            $.ajax({
                type: "POST",
                url:_lps_site_url+"user/create/class/save/",
...
function page_li_click(type,page,bools){
    $.ajax({
        type: "GET",
        url: _lps_site_url+"common/course/list",
        ...
        success: function(data){
            ...
            $(data).each(function(i,val) {
                    ...
                    str +='<a href="'+_lps_site_url+'course/'+val.course_id+'/recent/play/"><dl><dt><div style="width:275px;height:174px;">'
...
function login_popup(msg){
    _trackData.push(['addaction','登录','来源'+location.pathname]);
    $('#id_account_l').val("");
    $('#id_password_l').val("");
    $('#id_email').val("");
    $('#id_password').val("");
...
function secureHTML(unsafe) {
    return unsafe
    .replace(/<\s*(script.*?)>/ig,"&lt;$1&gt;")
    .replace(/<\s*(\/script)>/ig,"&lt;$1&gt;")
    .replace(/<\s*(iframe.*?)>/ig,"&lt;$1&gt;")
    .replace(/<\s*(\/iframe)>/ig,"&lt;$1&gt;")
}

function zytimer(){
    var d = new Date();
    var ddd=new Date(d.setDate(d.getDate()+2)),
    dd = ddd.getDay(),
    friday = new Date((5 - dd)*(3600*24*1000)+ddd.getTime());
    friday.setHours(0);
    friday.setMinutes(0);
    friday.setSeconds(0);
    var zdate=new Date();
    if(zdate.getDay()==5){
        zdate=new Date(zdate.getFullYear(),zdate.getMonth(),zdate.getDate(),0,0,0)
    }
    var ts = (friday) - (new Date());//计算剩余的毫秒数
    ...
    mm = checkTime(mm);
    ss = checkTime(ss);
    document.getElementById("timer").innerHTML = dd + "天" + hh + "时" + mm + "分";
}
function checkTime(i){
   if (i < 10) {
       i = "0" + i;
...
function openjifen(){
    $('#new_jifen').on('show.bs.modal', function () {
        setTimeout(function(){$(".zy_jifen_img").show().attr("src","/static/images/goldDown.gif?"+new Date().getTime());},1000);
        setTimeout(function(){
        $(".zy_jifen_img").hide();
        },2600)
    });
    $('#new_jifen').modal('show');
    $(".zy_jifen .close").click(function(){
        $('#new_jifen').modal('hide');
    })
    setTimeout(function(){$('#new_jifen').modal('hide');},4400)
}
